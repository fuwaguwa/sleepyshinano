...
import type { ShinanoUserVote } from "../../vote/types/UserVote";
import { BOORU_BLACKLIST, BOORU_CONFIG, BOORU_QUERY, PAGE_SELECTION } from "../constants";
import { UserBooruModel } from "../models/UserBooru";
import type { BooruPost, BooruSearchOptions, BooruSite, GelbooruPostResponse } from "../types/API";
import type {
...
export async function getUserVoteInfo(userId: string): Promise<ShinanoUserVote | null> {
  return await UserVoteModel.findOne({ userId });
}

async function extractUserVoteInfo(userId: string): Promise<BooruUserVoteInfo> {
  const voteInfoModel = await getUserVoteInfo(userId);
  return voteInfoModel
    ? {
        voteCreatedTimestamp: voteInfoModel.voteCreatedTimestamp,
        voteExpiredTimestamp: voteInfoModel.voteExpiredTimestamp,
      }
    : { voteCreatedTimestamp: null, voteExpiredTimestamp: null };
}

function shouldShowLoadMoreButton(interaction: ProcessableInteraction): boolean {
...
async function checkUserVoteStatus(userId: string, cachedVoteInfo: BooruUserVoteInfo | null): Promise<boolean> {
  ...
  const voteInfo: BooruUserVoteInfo = await extractUserVoteInfo(userId);
...
export async function queryBooru(
  ...
  let user = await UserBooruModel.findOne({ userId });
  if (!user) user = await UserBooruModel.create({ userId, booruState: new Map() });
  ...
  for (let iteration = 0; iteration < BOORU_QUERY.maxFetchRetries; iteration++) {
    ...
    return { post: selected, userVoteInfo: await extractUserVoteInfo(user.id) };
  }
  ...
  return { post: null, userVoteInfo: await extractUserVoteInfo(user.id) };
}

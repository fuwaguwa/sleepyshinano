...
import type { TopggVoteCheck } from "../types/Topgg";
...
export class DeveloperCommand extends Subcommand {
  ...
  public async subcommandVote(interaction: Subcommand.ChatInputCommandInteraction) {
    ...
    const voteUser = await UserVoteModel.findOne({ userId: user.id }).lean();
    ...
    collector.on("collect", async i => {
      ...
      // Update database
      const currentTime = getCurrentTimestamp();
      await UserVoteModel.findOneAndUpdate(
        { userId: user.id },
        {
          $set: {
            userId: user.id,
  ...
  public async subcommandBLCheck(interaction: Subcommand.ChatInputCommandInteraction) {
    ...
    const user = await UserModel.findOne({ userId: targetUser.id }).lean();

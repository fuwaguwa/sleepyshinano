...
import { KEMONO_BASE_URL } from "../../lib/constants";
import { kemonoSearchCreator } from "../../lib/kemono";
...
function generatePostContentDisplay(
  ...
  const attachments = post.attachments.filter(att => !att.endsWith(".bin"));
  container.logger.debug(attachments);
  for (let i = 0; i < attachments.length; i += 10) {
    const pageAttachments = attachments.slice(i, i + 10);

    const gallery = new MediaGalleryBuilder();

    for (const attachment of pageAttachments) gallery.addItems([{ media: { url: attachment } }]);
...
export class KemonoCommand extends Subcommand {
        ...
        .addSubcommand(command =>
          command
            ...
            .addStringOption(option =>
              option.setName("name").setDescription("The name of the creator").setRequired(true)
            )
        )
    );
  }
  ...
  public async subcommandCreator(interaction: Subcommand.ChatInputCommandInteraction) {
    ...
    const searchResults = await kemonoSearchCreator(creatorName);
    ...
    this.container.logger.debug(searchResults[0]);
  ...
  private async showPostsSelection(
    ...
    const postsSelections = generatePostsSelections(creator, posts as PostListItem[], interaction.user.id, true);
